-- Add missing implicate_pain field to opportunities table
-- This field is required for the MEDDPICC scoring algorithm (20% weight)

-- Add the implicate_pain column
ALTER TABLE opportunities 
ADD COLUMN implicate_pain TEXT;

-- Update the database function to include implicate_pain in scoring
...
DECLARE
    ...
    keyword TEXT;
BEGIN
    -- Get opportunity data including the new implicate_pain field
    SELECT 
        metrics, economic_buyer, decision_criteria, decision_process,
        paper_process, identify_pain, implicate_pain, champion, competition
    INTO opp_record
    FROM opportunities 
    WHERE id = opp_id;
...
COMMENT ON COLUMN opportunities.implicate_pain IS 'Implicate Pain pillar for MEDDPICC scoring - helps prospects understand the full impact of their pain (20% weight)';
