...
// Auth handled via services/hooks; no direct client access needed here
...
export default function OpportunityFormTabbed({ opportunity, opportunityId, mode }: OpportunityFormProps) {
  const router = useRouter()
  const { handleAsyncOperation } = useErrorHandler()

  // Core form state
  const [activeTab, setActiveTab] = useState<'details' | 'peak' | 'meddpicc'>('details')
  ...
  const [ownerId, setOwnerId] = useState<string>((opportunityRecord?.assigned_to as string) || '')
  const formDefaults = opportunity ? {
    ...
    assigned_to: (opportunityRecord.assigned_to as string) || '',
    description: (opportunityRecord.description as string) || '',
  } : {
    name: '',
    contact_id: '',
    company_id: '',
  ...
  const loadOwners = useCallback(async () => { /* owners handled by UserSelect */ }, [])
  ...
  const loadOpportunity = useCallback(async () => {
    ...
    try {
      ...
      reset({
        ...
        assigned_to: (resetDataRecord.assigned_to as string) || '',
        description: (resetDataRecord.description as string) || ''
      })
      setOwnerId((resetDataRecord.assigned_to as string) || '')
  ...
  }, [loadContacts, loadCompanies, loadOwners, loadOpportunity, mode, opportunityId])
            ...
            <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as typeof activeTab)} className="space-y-6">
              ...
              <TabsContent value="details" className="space-y-6">
                ...
                <div className="rounded-md bg-blue-50 border border-blue-100 p-3 text-sm text-blue-800">
                  Looking for Deal Value or Expected Close Date? Manage them in the PEAK tab.
                  <button
                    type="button"
                    onClick={() => setActiveTab('peak')}
                    className="ml-2 inline-flex items-center text-blue-700 underline hover:text-blue-800"
                  >
                    Go to PEAK
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                  <div>
                    ...
                    {errors.contact_id && (
                      <p className="mt-2 text-sm text-red-600">{errors.contact_id.message}</p>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      readOnly
                      value={
                        ((opportunityRecord?.region as string) ||
                          (opportunityRecord?.territory_name as string) ||
                          (opportunityRecord?.territory as string) ||
                          '') as string
                      }
                ...
                {/* Moved Deal Value and Expected Close Date to PEAK tab */}
