...
export class OpportunityAPIEnhanced {
  ...
  /**
   * Get all opportunities with enhanced error handling
   */
  ...
  /**
   * Get single opportunity with enhanced error handling
   */
  ...
  /**
   * Create opportunity with comprehensive validation
   */
  ...
  /**
   * Update opportunity with comprehensive validation and field mapping
   */
   ...
   */
  async updatePEAKData(id: string, peakData: PEAKData): Promise<ApiResponse<Opportunity>> {
    try {
      if (!id) {
        return { data: null, error: { message: 'Opportunity ID is required' } }
      }
  ...
  /**
   * Update MEDDPICC data specifically
   */
  ...
  async deleteOpportunity(id: string): Promise<{ error: ApiError | null }> {
    try {
      if (!id) {
        return { error: { message: 'Opportunity ID is required' } }
      }
  ...
  /**
   * Search opportunities with enhanced error handling
   */
  async searchOpportunities(query: string): Promise<ApiResponse<OpportunityWithDetails[]>> {
    try {
      ...
      console.error('Unexpected error searching opportunities:', error)
      return { data: null, error: normalizeError(error) }
    }
  }

  /**
   * Get opportunities by stage with enhanced error handling
   */
  async getOpportunitiesByStage(stage: string): Promise<ApiResponse<OpportunityWithDetails[]>> {
    try {
      if (!stage) {
        return { data: null, error: { message: 'Stage is required' } }
      }
      ...
      console.error('Unexpected error fetching opportunities by stage:', error)
      return { data: null, error: normalizeError(error) }
    }
  }

  /**
   * Update PEAK stage with validation
   */
  async updatePeakStage(id: string, stage: 'prospecting' | 'engaging' | 'advancing' | 'key_decision'): Promise<ApiResponse<Opportunity>> {
    try {
      ...
      if (error) {
        console.error('Error updating peak stage:', error)
        return { data: null, error: normalizeError(error) }
      }

      return { data, error: null }
    } catch (error) {
      console.error('Unexpected error updating peak stage:', error)
      return { data: null, error: normalizeError(error) }
    }
  }

  /**
   * Get pipeline summary with enhanced error handling
   */
  async getPipelineSummary(): Promise<ApiResponse<PipelineSummary>> {
    try {
      ...
      console.error('Unexpected error fetching pipeline summary:', error)
      return { data: null, error: normalizeError(error) }
    }
  }
}

export const opportunityAPIEnhanced = new OpportunityAPIEnhanced()
